...
Evaluate the effectiveness and detection capabilities of Wazuh by installing it and an agent on both on a Win 10 and Linux OS, and simulating various cyber-attacks to observe and analyze the alerts generated by Wazuh. This will be divided into different parts:
...
For my Linux agent, I will installing it on a host running Ubuntu. To do this we navigate to the window to add agent and select the same paramenters as we did with the Win 10, only this time for Linux:
